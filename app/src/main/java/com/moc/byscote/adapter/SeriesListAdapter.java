...
public class SeriesListAdapter extends RecyclerView.Adapter<SeriesListAdapter.PersonViewHolder> {
    ...
    public SeriesListAdapter(Context context , List<SeriesList> seriesList_Items) {
    ...
    public void onBindViewHolder(final PersonViewHolder personViewHolder, final int i) {
      ...
      //  personViewHolder.tv_category.setBackgroundResource(categoryList_Items.get(i).getImg_src());
        if(i==0){
            Picasso.get().load(R.mipmap.a1).into(personViewHolder.img_series);
            personViewHolder.tv_series_title.setText("Gohn Lenon Rest in Peace");
        }else if(i==1){
            Picasso.get().load(R.mipmap.a2).into(personViewHolder.img_series);
            personViewHolder.tv_series_title.setText("Five Story");
        }else  if(i==2){
            Picasso.get().load(R.mipmap.a3).into(personViewHolder.img_series);
            personViewHolder.tv_series_title.setText("Wildest Dreams");
        }else {
